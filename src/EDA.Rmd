...
# Plot vertical histogram for both ABV and IBU
mergedDataIBU = mergedData %>% group_by(State) %>% summarise(median = median(IBU))
mergedDataABV = mergedData %>% group_by(State) %>% summarise(median = median(ABV))
mergedDataABV = as.data.frame(mergedDataABV)
mergedDataIBU = as.data.frame(mergedDataIBU)
ABV_IBU_median = merge(mergedDataABV, mergedDataIBU, by = "State")
colnames(ABV_IBU_median)[2] <- "median_ABV"
colnames(ABV_IBU_median)[3] <- "median_IBU"
ABV_IBU_median$median_ABV = ABV_IBU_median$median_ABV * -1000

longTest = gather(ABV_IBU_median, moc, value, median_ABV:median_IBU)

longTest %>% ggplot(aes(x = value, y = State, fill = moc)) + geom_bar(stat = "identity") + scale_x_continuous(breaks = c(-60, 0, 60), labels = c(60, 0, 60)) + theme_bw() + scale_fill_discrete(name = "Measure of Center", labels = c("Median ABV", "Median IBU"))
```
...
## 9. Knocking your socks off with the top 10 beer styles
[insert explicit answer]
```{r}
# Wrangling the data
...
## 9.1. Correlation of ABV and IBU of the top 10 beer styles
```{r}
```
